...
import { Facebook, DollarSign, MousePointerClick, Eye, Users, Target, Activity, Sparkles } from "lucide-react";
...
export const metaAdsDef: PlatformDef = {
  ...
  metrics: [
    { key: "spend",       column: "spend",       label: "Investimento", format: "currency", aggregation: { kind: "sum" }, icon: DollarSign,        positiveIsGood: true,  description: "Somatório do gasto declarado pela Meta." },
    { key: "reach",       column: "reach",       label: "Alcance",      format: "int",      aggregation: { kind: "sum" }, icon: Users,             positiveIsGood: true,  description: "Soma do reach diário entregue pela Meta. Pessoas únicas POR DIA." },
    { key: "impressions", column: "impressions", label: "Impressões",   format: "int",      aggregation: { kind: "sum" }, icon: Eye,               positiveIsGood: true,  description: "Total de impressões servidas." },
    { key: "clicks",      column: "clicks",      label: "Cliques",      format: "int",      aggregation: { kind: "sum" }, icon: MousePointerClick, positiveIsGood: true,  description: "Cliques registrados no período." },
  ],
  ...
  kpis: [
    ...
    { key: "cpm",       label: "CPM médio",   format: "currency", positiveIsGood: false, compute: (t) => f.cpm(t.spend, t.impressions),    icon: Sparkles,  description: "Spend ÷ Impressions × 1000." },
    { key: "frequency", label: "Frequência",  format: "decimal",  positiveIsGood: true,  compute: (t) => f.frequency(t.impressions, t.reach), description: "Impressions ÷ Reach do período." },
  ],
  charts: [
    {
