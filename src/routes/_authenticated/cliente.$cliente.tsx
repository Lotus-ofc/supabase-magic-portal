  ...
  queryOptions({
    ...
    queryFn: async (): Promise<DailyView[]> => {
      const view =
        ...
        platform === "google_ads" ? "vw_google_ads_diario" :
        platform === "ga4"        ? "vw_ga4_diario"        :
        platform === "instagram"  ? "vw_instagram_diario"  : null;
      if (!view) return [];
      const since = new Date();
...
function ClienteBody({ cliente, days }: { cliente: string; days: PeriodDays }) {
  ...
  // KPIs canônicos — só mostramos quando há base.
  return (
    <div className="space-y-7">
      {/* HERO KPIs */}
      ...
      {/* COMPARATIVOS rápidos: 7d/30d/90d (apenas para evidenciar evolução) */}
