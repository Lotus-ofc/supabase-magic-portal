/**
 * Envio de convites Supabase Auth com redirectTo expl├¡cito.
 * Somente servidor.
 */
  ...
  if (!report.invites_allowed) {
    throw new AuthInviteError(
      report.message ??
        ...
        "Configura├º├úo de autentica├º├úo inv├ílida para envio de convites.",
      "invite_config",
    );
  }
...
export async function sendAuthInviteEmail(
  ...
  if (error) {
    if (isInviteConfigError(error.message)) {
      const msg = `Link de convite rejeitado (${redirectTo}). Configure APP_URL no servidor e adicione esta URL em Supabase ÔåÆ Authentication ÔåÆ Redirect URLs. Detalhe: ${error.message}`;
    ...
    if (isUserAlreadyExists(error.message)) {
      const msg =
        "Este e-mail j├í est├í cadastrado. Use ÔÇ£Reenviar e-mailÔÇØ na lista de usu├írios.";
      auditFailure({ email, action: "invite", app_url: appUrl, redirect_to: redirectTo }, msg);
  ...
  if (!userId) {
    const msg = "O Supabase n├úo retornou o id do usu├írio ap├│s o convite.";
    auditFailure({ email, action: "invite", app_url: appUrl, redirect_to: redirectTo }, msg);
    throw new AuthInviteError(msg, "invite_failed");
  }
...
/** Reenvia e-mail de convite/acesso para usu├írio j├í cadastrado (sem recriar conta). */
export async function resendAuthInviteEmail(
  ...
  if (invited.error) {
    ...
    if (isUserAlreadyExists(invited.error.message)) {
      ...
      const msg =
        "N├úo foi poss├¡vel reenviar o e-mail. Verifique o endere├ºo e tente novamente em alguns minutos.";
      auditFailure(
        { email, user_id: userId, action: "resend", app_url: appUrl, redirect_to: redirectTo },
        msg,
      );
  ...
  const msg = "N├úo foi poss├¡vel reenviar o convite.";
  auditFailure(
    { email, user_id: userId, action: "resend", app_url: appUrl, redirect_to: redirectTo },
    msg,
  );
...
export async function sendPasswordResetEmail(
  ...
  if (error) {
    if (isInviteConfigError(error.message)) {
      throw new AuthInviteError(
        `Link de recovery rejeitado (${recoveryRedirect}). Adicione esta URL em Supabase ÔåÆ Redirect URLs. Detalhe: ${error.message}`,
        "invite_config",
      );
    }
    throw new AuthInviteError(
